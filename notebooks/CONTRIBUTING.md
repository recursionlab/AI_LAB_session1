...
2. **Justify the change:** Explain _why_ the change is necessary and how it aligns with the core philosophy.
...
## 6. Cross-Formalism Mapping Standards

To ensure clarity and avoid overclaiming, use this taxonomy and standards.

### 6.1 Taxonomy (use words precisely)

* **Analogy:** “feels similar”, no committed mapping.
* **Correspondence:** role-mapping, no claim of structure preservation.
* **Homomorphism:** structure-preserving map (not necessarily bijective). Proof required.
* **Isomorphism:** bijective structure-preserving map. Proof required.

### 6.2 Standards

* **Purpose:** Clearly state the objective (stress-test, implementation lens, proof target).
...
* **Interpretation:** Explain the implications of the mapping for the Ψ formalism, highlighting insights gained or structural validations achieved.
* **Claim strength:** Label the mapping as Analogy / Correspondence / Homomorphism / Isomorphism.
* **Limitations:** State what is _not_ preserved (structure, semantics, dynamics, units).
* **Proof gate (if homomorphism/isomorphism):** Include assumptions + proof sketch.
* **Future work:** Outline steps to upgrade correspondence → homomorphism → isomorphism.
...
**The Recursive Verification Process:** An equation begins at Level 1. If it fails any check at any level, it is returned for revision and must re-start the entire process from Level 1. This ensures holistic integrity after any change.

### 9.1 Where verification outcomes live (logging rule)

Every canonical equation page must link to a dedicated verification log page.

**Rule:**

* Create a page named `Verification Log: <Equation page title>`.
* Keep it in `notebooks/` (or a `notebooks/verification-logs/` group if/when we add it).
* The equation page has a short **Verification** section:
  * current Level (1–5)
  * status (PASS/FAIL/PENDING)
  * link to the log page

This prevents verification state from getting buried in long equation pages.
...
#### Level 4: External Structural Validation

* **Objective:** Validate structure by mapping it to established formalisms, with explicit claim strength (analogy/correspondence/homomorphism/isomorphism).
* **Verification Steps:**
  1. Perform a structural mapping to a relevant external formalism (e.g., Lagrangian mechanics, QFT, modal logic) as documented in [Cross-Formalism Structural Mappings - v0.1](INTERPRETATION/Structural_Isomorphisms_v0.1.md).
